...
class Encoder(nn.Module):
    """
    Conditional encoder that maps concatenated (input, condition) vectors
    to the parameters of a Gaussian latent distribution.
    """
    ...
    def __init__(self, p, p_cond, latent_dim, p_encoder_lst, hidden_act='elu', use_batch_norm=True):
        ...
        if use_batch_norm:
            # When using batch norm, biases are redundant in linear layers
            ...
            # Separate affine projections for mean and log-variance
    ...
    def get_activation(self, act_name):
        """Return an activation module based on the provided name."""
        if act_name == 'elu':
            return nn.ELU()
        elif act_name == 'relu':
            return nn.ReLU()
    ...
    def forward(self, x, cond):
        """
        Forward pass that concatenates input with condition
        and outputs latent mean/log-variance.
        """
...
class Decoder(nn.Module):
    """
    Conditional decoder that reconstructs inputs from latent vectors
    concatenated with condition embeddings.
    """
    ...
    def forward(self, z, cond):
        """
        Concatenate latent sample with condition and decode back to input space.
        """
...
class CVAE(nn.Module):
    """
    Conditional Variational Autoencoder tying together the encoder/decoder.
    """

    def __init__(self, p, p_cond, latent_dim, p_encoder_lst, p_decoder_lst,
                 hidden_act='elu', output_act='relu', use_batch_norm=True):
    ...
    def reparameterize(self, z_mean, z_log_var):
        """
        Reparameterization trick to sample latent variables from the encoder output.
        """
    ...
    def forward(self, x, cond):
        """
        Encode input, sample latent vector, then decode under the same condition.
        """
...
def weighted_loss_function(x, x_reconstructed, z_mean, z_log_var, weights, beta=1.0):
    """
    Weighted VAE loss allowing per-sample importance weights.
    """
