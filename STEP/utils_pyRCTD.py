...
def run_RCTD(
    ...
    """
    Main orchestration routine: adjusts platform effects, fits bulk proportions,
    selects sigma, and finally fits per-pixel cell-type weights.
    """
    if drop:
    ...
    RCTD = effectBalance(RCTD, out_dir, hs_ST, loggings=loggings, alpha=alpha, beta=beta)
    if process_log:
...
def fitBulk(RCTD, loggings=None):
    """
    Decompose bulk-like aggregated counts to estimate global cell-type proportions
    and produce a renormalized reference matrix.
    """
    bulkData = prepareBulkData(
        RCTD['cell_type_info']['info']['cell_type_means'],
        RCTD['reconSpatialRNA'],
        RCTD['internal_vars']['gene_list_bulk']
    )
    loggings.info('fitBulk: decomposing bulk')
    decompose_results = decompose_full(
        bulkData['X'],
        RCTD['reconSpatialRNA']['nUMI'].sum().item(),
        np.array(bulkData['b']),
        verbose=False,
        constrain=False,
...
def get_norm_ref(puck, cell_type_means, gene_list, proportions):
    """
    Renormalize reference cell-type means so that the weighted average matches
    the observed bulk composition from ST data.
    """
    bulk_vec = puck['counts'].sum(1)
    weight_avg = (cell_type_means.loc[gene_list, :] *
                  (proportions / proportions.sum()).values.squeeze()).sum(1)
    weight_avg = np.where(weight_avg == 0, 1e-6, weight_avg)
    target_means = bulk_vec.loc[gene_list] / puck['nUMI'].sum().item()
    cell_type_means_renorm = cell_type_means.loc[gene_list, :] / (
        weight_avg / target_means).values.squeeze()[:, None]
    return cell_type_means_renorm


def prepareBulkData(cell_type_means, puck, gene_list, MIN_OBS=10):
    """
    Build the design matrix X and observation vector b for bulk decomposition.
    """
    bulk_vec = puck['counts'].sum(1)
...
def effectBalance(RCTD, out_dir, hs_ST=False, loggings=None, alpha=1.0, beta=1.0):
    """
    Perform platform-effect correction using a CVAE (conditional VAE) domain
    adaptation between ST and SC data.
    """
    def mse_loss(array1, array2):
        return np.mean((array1 - array2) ** 2)

    args = Args()
    args.device = RCTD['config']['device']
    args.beta = beta
    ...
    model_path = os.path.join(out_dir, 'model/model.pth')
    if os.path.exists(model_path):
        loggings.info('Loading existing model')
        cvae = torch.load(model_path)

        p = len(RCTD['internal_vars']['gene_list_bulk'])
        p_cond = 1
        latent_dim = n_celltype * 3
        hidden_dim = list(np.floor(
    ...
    new_st = np.rint((new_st / new_st.sum(axis=1)[:, None]).T *
                     RCTD['spatialRNA']['counts'].sum(axis=0).values[None, :])
    new_st = pd.DataFrame(
        new_st,
        columns=RCTD['spatialRNA']['counts'].columns,
        index=RCTD['internal_vars']['gene_list_bulk']
    )

    if alpha < 1.0:
        raw_st = RCTD['spatialRNA']['counts'].loc[RCTD['internal_vars']['gene_list_bulk']]
        new_st = (1.0 - alpha) * raw_st + alpha * new_st

    RCTD['reconSpatialRNA'] = RCTD['spatialRNA'].copy()
    RCTD['reconSpatialRNA']['counts'].loc[RCTD['internal_vars']['gene_list_bulk']] = new_st
    return RCTD


def calc_Q_all(x, bead, likelihood_vars=None):
    """
    Compute interpolated likelihood components (r1, r2, r3) by locating the
    appropriate entries in the Q-matrix for given x and bead values.
    """
...
def get_d1_d2(B, prediction, likelihood_vars=None):
    """
    Compute first and second derivatives (d1, d2) of the negative log-likelihood
    w.r.t. predicted means for use in IRWLS.
    """
    X_vals = likelihood_vars['X_vals']
    Q_mat = likelihood_vars['Q_mat']
    ...
    Q_d2 = 1 / (x ** 2) * ((bead + 1) * (bead + 2) * Q_k2 -
                           bead * (2 * (bead + 1) * Q_k1 - (bead - 1) * Q_k))
...
def calc_Q_k(x, bead, likelihood_vars=None):
    """
    Convenience function: single interpolation for a specific bead/x pair.
    """
    X_vals = likelihood_vars['X_vals']
    Q_mat = likelihood_vars['Q_mat']
    K_val = likelihood_vars['K_val']
...
def calc_Q_k_GPU(x, bead, likelihood_vars=None):
    """
    GPU version of calc_Q_k using torch tensors.
    """
    X_vals = likelihood_vars['X_vals']
    Q_mat = likelihood_vars['Q_mat']
    K_val = likelihood_vars['K_val']
...
def calc_log_l_vec(lamb, Y, return_vec=False, likelihood_vars=None, GPU=False):
    """
    Evaluate negative log-likelihood (optionally returning element-wise values).
    """
    if GPU:
    ...
    return log_l_vec if return_vec else log_l_vec.sum()


def chooseSigma(prediction, counts, Q_mat_all, X_vals, sigma):
    """
    Grid search over nearby sigma values to minimize negative log-likelihood.
    """
    X = prediction.values.T.reshape(-1)
    X = np.maximum(X, 1e-4)
    Y = counts.T.reshape(-1)

    num_sample = min(1_000_000, X.shape[0])
...
def get_der_fast(S, B, S_mat, gene_list, prediction, bulk_mode=False, likelihood_vars=None):
    """
    Compute gradient and Hessian (per IRWLS iteration) using either bulk or
    spatial likelihood formulation.
    """
    if bulk_mode:
...
def psd(H):
    """
    Project a matrix onto the PSD cone by clipping negative eigenvalues.
    """
    eigvals, eigvecs = np.linalg.eig(H)
    epsilon = 1e-3
    if H.shape[0] == 1:
        P = eigvecs @ np.maximum(eigvals, epsilon) @ eigvecs.T
    else:
        P = eigvecs @ np.diag(np.maximum(eigvals, epsilon)) @ eigvecs.T
    return P


def solveWLS(S, B, S_mat, initialSol, nUMI, bulk_mode=False, constrain=False, likelihood_vars=None, solver='osqp'):
    """
    Single IRWLS inner loop: solve quadratic subproblem for weights.
    """
...
def solveIRWLS_weights(
    ...
    """
    Iteratively reweighted least squares (IRWLS) solver for cell-type weights.
    """
    if not bulk_mode:
        K_val = likelihood_vars['K_val']
        B = np.copy(B)
...
def decompose_full_ray(args):
    """
    Ray-remote wrapper around solveIRWLS_weights for parallel pixel fits.
    """
...
def decompose_full(
    ...
    """
    Convenience wrapper calling solveIRWLS_weights (single thread).
    """
    results = solveIRWLS_weights(
        cell_type_profiles,
        bead,
...
def decompose_batch(
    ...
    """
    Batch IRWLS for multiple pixels using Ray for parallelism.
    """
    inp_args = []
    weights = []
    for i in range(beads.shape[0]):
        ...
        bead[bead > K_val] = K_val
        inp_args.append((
            cell_type_means.loc[gene_list, :] * nUMI[i],
            nUMI[i],
            bead,
            constrain,
            OLS,
...
def SpatialRNA(coords, counts, nUMI=None):
    """
    Build a SpatialRNA dictionary after ensuring all barcodes are shared.
    """
    barcodes = list(set(coords.index) & set(counts.columns) & set(nUMI.index))
    if len(barcodes) == 0:
        raise ValueError(
            'SpatialRNA: coords, counts, and nUMI do not share any barcode names.'
        )
    if len(barcodes) < max(coords.shape[0], counts.shape[1], nUMI.shape[0]):
        raise ValueError(
            'SpatialRNA: some barcodes were not mutually shared and were removed.'
        )
    return {'coords': coords, 'counts': counts, 'nUMI': nUMI}


def Reference(counts, cell_types, nUMI=None, n_max_cells=10000, loggings=None):
    """
    Build a reference dictionary with optional downsampling to limit per-type size.
    """
    reference_dict = {'cell_types': cell_types, 'counts': counts, 'nUMI': nUMI}
    cur_count = reference_dict['cell_types'].value_counts().max()
    if cur_count > n_max_cells:
        loggings.info(
            'Reference: number of cells per cell type is {}, larger than maximum {}. '
            'Downsampling to {}.'.format(cur_count, n_max_cells, n_max_cells)
        )
...
def create_downsampled_data(reference, cell_types_keep=None, n_samples=10000):
    """
    Downsample each cell type to at most n_samples cells.
    """
...
def get_cell_type_info(counts, cell_types, nUMI, cell_type_names=None):
    """
    Compute mean expression per gene for each cell type in the reference.
    """
...
def process_cell_type_info(reference, cell_type_names, CELL_MIN=25, loggings=None):
    """
    Validate reference cell counts and produce averaged cell-type profiles.
    """
    loggings.info("Begin: process_cell_type_info")
    loggings.info(f"process_cell_type_info: number of cells in reference: {reference['counts'].shape[1]}")
    loggings.info(f"process_cell_type_info: number of genes in reference: {reference['counts'].shape[0]}")
    ...
    if reference['cell_types'].value_counts().min() < CELL_MIN:
        loggings.info(
            f"process_cell_type_info error: need at least {CELL_MIN} cells for each cell type."
        )
...
def restrict_counts(puck, gene_list, UMI_thresh=1, UMI_max=20000):
    """
    Filter spatial beads by total UMI range and restrict genes to gene_list.
    """
    nUMI = puck['nUMI'].to_numpy()
    keep_loc = (nUMI >= UMI_thresh) & (nUMI <= UMI_max)
...
def get_de_genes(cell_type_info, puck, fc_thresh=1.25, expr_thresh=.00015, MIN_OBS=3, loggings=None):
    """
    Identify DE genes per cell type for regression/bulk normalization.
    """
    total_gene_list = []
    epsilon = 1e-9
    ...
    if gene_list.shape[0] == 0:
        raise ValueError(
            "get_de_genes: no common genes between SpatialRNA and Reference objects."
        )
    ...
    for cell_type in cell_type_info['cell_type_names']:
        ...
        logFC = np.log(cell_type_info['cell_type_means'].loc[gene_list, cell_type] + epsilon) - \
                np.log(other_mean + epsilon)
        type_gene_list = np.where(
            ((logFC > fc_thresh) &
             (cell_type_info['cell_type_means'].loc[gene_list, cell_type] > expr_thresh)) == True
        )[0]
        loggings.info(f"get_de_genes: {cell_type} found DE genes: {type_gene_list.shape[0]}")
    ...
    loggings.info(f"get_de_genes: total DE genes: {gene_list.shape[0]}")
    return gene_list


def restrict_puck(puck, barcodes):
    """
    Keep only specific barcodes (spots) in counts/nUMI/coords.
    """
...
def create_RCTD(
    ...
    """
    Construct the full RCTD object: select gene lists, normalize counts, and
    stash metadata needed downstream.
    """
    config = {
    ...
    puck_original = restrict_counts(
        spatialRNA,
        np.array(spatialRNA['counts'].index),
        UMI_thresh=config['UMI_min'],
        UMI_max=config['UMI_max']
    )

    loggings.info('create_RCTD: getting regression differentially expressed genes: ')
    if markers_genes is not None:
        gene_list_reg = markers_genes
    else:
        if hs_ST or spatialRNA['counts'].shape[0] < 1000:
            gene_list_reg = np.intersect1d(
                spatialRNA['counts'].index.values,
                reference['counts'].index.values
            )
            loggings.info(f"get_de_genes: total DE genes: {gene_list_reg.shape[0]}")
        else:
    ...
    if gene_list_reg.shape[0] == 0:
        raise ValueError("create_RCTD: 0 regression DE genes found")

    loggings.info('create_RCTD: getting platform effect normalization DE genes: ')
    if markers_genes is not None:
        gene_list_bulk = markers_genes
    else:
        if hs_ST or spatialRNA['counts'].shape[0] < 1000:
            gene_list_bulk = np.intersect1d(
                spatialRNA['counts'].index.values,
                reference['counts'].index.values
            )
            loggings.info(f"get_de_genes: total DE genes: {gene_list_bulk.shape[0]}")
        else:
    ...
    if gene_list_bulk.shape[0] == 0:
        raise ValueError("create_RCTD: 0 bulk DE genes found")
    ...
    RCTD = {
        'spatialRNA': puck,
        'reconSpatialRNA': None,
        'reference': reference,
        'config': config,
        'cell_type_info': cell_type_info,
        'internal_vars': internal_vars,
        'imageInfo': {'partion': None, 'features': None, 'polygon': None}
    }
    return RCTD
...
def choose_sigma_c(RCTD, Q_mat_all, X_vals_loc, loggings=None):
    """
    Iteratively solve for sigma that best matches observed ST beads by alternating
    decomposition and likelihood evaluation.
    """
    ...
    if N_fit == 0:
        raise ValueError(
            'choose_sigma_c: N_fit is 0. Decrease UMI_min_sigma.'
        )
    fit_ind = np.random.choice(
        puck['nUMI'][puck['nUMI'] > MIN_UMI].index,
        N_fit,
        replace=False
    )
    beads = puck['counts'].loc[RCTD['internal_vars']['gene_list_reg'], fit_ind].values.T
    loggings.info(f'chooseSigma: using initial Q_mat with sigma = {sigma/100}')
    likelihood_vars = {
        'Q_mat': Q_mat_all[str(sigma)],
        'X_vals': X_vals_loc,
        'N_X': Q_mat_all[str(sigma)].shape[1],
    ...
    for _ in np.arange(RCTD['config']['N_epoch']):
        ...
        for i, res in enumerate(results):
            weights[i] = res.squeeze()

        prediction = RCTD['cell_type_info']['renorm']['cell_type_means'].loc[
            RCTD['internal_vars']['gene_list_reg'], :
        ] @ weights.T * (puck['nUMI'].loc[fit_ind]).values.squeeze()[None, :]

        loggings.info(
            f'Likelihood value: {calc_log_l_vec(prediction.values.T.reshape(-1), beads.reshape(-1), likelihood_vars=likelihood_vars)}'
        )
        sigma_prev = sigma
        sigma = chooseSigma(prediction, beads.T, Q_mat_all, likelihood_vars['X_vals'], sigma)
        loggings.info(f'Sigma value: {sigma/100}')
        if sigma == sigma_prev:
            break
...
def fitPixels(RCTD, doublet_mode='full', loggings=None):
    """
    Final per-spot decomposition: solve for cell-type fractions across the ST grid.
    """
    ...
    if doublet_mode == 'full':
        ...
        results = decompose_batch(
            np.array(RCTD['reconSpatialRNA']['nUMI']).squeeze(),
        ...
        RCTD['results'] = pd.DataFrame(
            weights,
            index=RCTD['reconSpatialRNA']['counts'].columns,
